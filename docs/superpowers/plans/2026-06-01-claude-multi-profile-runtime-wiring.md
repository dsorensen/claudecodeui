...
**As-built follow-up (surfaced by the final cumulative review, deferred to Plan 3):** `bucketSessionRowsByProvider` in `server/modules/projects/services/projects-with-sessions-fetch.service.ts` — a file Plan 2 did not touch — buckets DB session rows into a fixed base-provider record, so a `claude:*` instance-id row hits `if (!bucket) continue;` and is silently dropped from `getProjectsWithSessions` (the sidebar / project-list path) and from the watcher's `projects_updated` broadcast. Net effect with multiple profiles: a non-default profile's sessions are indexed (Task 4/6) but invisible in the list. The default single-`claude`-profile path is unaffected. This is frontend-facing list behavior in the same class as the deferred APIs below — see the Plan 3 item.
...
- **Un-collapse the three frontend-facing APIs** deferred from Plan 2: `sessions.service.listProviderIds()` → `string[]` of instance ids; `mcp.service.addMcpServerToAllProviders` results → `{ provider: string }`; and `bucketSessionRowsByProvider` (`projects-with-sessions-fetch.service.ts`) must stop dropping `claude:*` rows so per-profile sessions appear in the project/session list (the As-built follow-up above) — this requires the Plan 3 sidebar display decision (which bucket / instance-id badge). Also update the frontend `LLMProvider` mirror (`src/types/app.ts`) + keyed maps (`providerAuthStatus`, `FALLBACK_DEFAULT_MODEL`, `providerModelCatalog`) to key by full instance id while resolving type-level concerns (icon, model catalog) by `baseProvider`. (`sessions.service.listArchivedSessions` also leaks a raw instance id on the wire today — collapse or expose deliberately here too.)
